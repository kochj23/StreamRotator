#!/usr/bin/env python3
"""
Add RTSPGlassmorphicBackgroundView files to Stream Rotator Xcode project.
"""
...
"""
    ...
    # Find the main group (Stream Rotator folder) and add file references
    # Look for the Stream Rotator group with its children
    stream_rotator_group = re.search(r'([A-F0-9]{24}) /\* Stream Rotator \*/ = \{[^}]*?children = \((.*?)\);', content, re.DOTALL)
    if stream_rotator_group:
        group_uuid = stream_rotator_group.group(1)
        children_content = stream_rotator_group.group(2)
...
if __name__ == '__main__':
    project_path = '/Volumes/Data/xcode/Stream Rotator/Stream Rotator.xcodeproj/project.pbxproj'
    try:
