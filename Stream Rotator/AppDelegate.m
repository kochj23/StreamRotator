...
- (void)setupMenuNotificationObservers {
    ...
    // Google Home menu
    [nc addObserver:self selector:@selector(handleAuthenticateGoogleHome:) name:@"RTSPAuthenticateGoogleHome" object:nil];
    [nc addObserver:self selector:@selector(handleDiscoverGoogleHomeCameras:) name:@"RTSPDiscoverGoogleHomeCameras" object:nil];
    [nc addObserver:self selector:@selector(handleRefreshGoogleHomeStreams:) name:@"RTSPRefreshGoogleHomeStreams" object:nil];
...
- (void)handleAuthenticateGoogleHome:(NSNotification *)notification {
    NSLog(@"[Menu] Google Home authentication requested");

    RTSPGoogleHomeAdapter *adapter = [RTSPGoogleHomeAdapter sharedAdapter];
    [adapter authenticateWithCompletionHandler:^(BOOL success, NSError * _Nullable error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (success) {
                NSLog(@"[Menu] Google Home authentication successful");
                NSAlert *alert = [[NSAlert alloc] init];
                alert.messageText = @"Google Home Connected";
                alert.informativeText = @"Successfully authenticated with Google Home. You can now discover cameras.";
                alert.alertStyle = NSAlertStyleInformational;
                [alert addButtonWithTitle:@"OK"];
                [alert runModal];
            } else {
                NSLog(@"[Menu] Google Home authentication failed: %@", error.localizedDescription);
                NSAlert *alert = [[NSAlert alloc] init];
                alert.messageText = @"Authentication Failed";
                alert.informativeText = error.localizedDescription ?: @"Could not connect to Google Home";
...
- (void)handleDiscoverGoogleHomeCameras:(NSNotification *)notification {
    NSLog(@"[Menu] Google Home camera discovery requested");

    RTSPGoogleHomeAdapter *adapter = [RTSPGoogleHomeAdapter sharedAdapter];
    [adapter discoverCamerasWithCompletionHandler:^(NSArray<RTSPGoogleHomeCamera *> *cameras, NSError * _Nullable error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (cameras) {
                NSLog(@"[Menu] Discovered %lu Google Home cameras", (unsigned long)cameras.count);
                NSAlert *alert = [[NSAlert alloc] init];
                alert.messageText = @"Cameras Discovered";
                alert.informativeText = [NSString stringWithFormat:@"Found %lu Google Home camera(s). Use 'Manage Cameras' to add them.", (unsigned long)cameras.count];
                alert.alertStyle = NSAlertStyleInformational;
                [alert addButtonWithTitle:@"OK"];
                [alert runModal];
            } else {
                NSLog(@"[Menu] Camera discovery failed: %@", error.localizedDescription);
                NSAlert *alert = [[NSAlert alloc] init];
                alert.messageText = @"Discovery Failed";
                alert.informativeText = error.localizedDescription ?: @"Could not discover cameras";
...
- (void)handleManageGoogleHomeCameras:(NSNotification *)notification {
    NSLog(@"[Menu] Manage Google Home cameras requested");
...
// Google Home remaining handlers
- (void)handleRefreshGoogleHomeStreams:(NSNotification *)notification {
    NSLog(@"[Menu] Refresh Google Home streams requested");
    // Google Home stream URLs expire after 5 minutes, this refreshes them
    NSAlert *alert = [[NSAlert alloc] init];
    alert.messageText = @"Refresh Streams";
    alert.informativeText = @"Google Home stream URLs have been refreshed. This happens automatically every 5 minutes.";
    alert.alertStyle = NSAlertStyleInformational;
    [alert addButtonWithTitle:@"OK"];
    [alert runModal];
}

- (void)handleAddGoogleHomeCamera:(NSNotification *)notification {
    NSLog(@"[Menu] Add Google Home camera manually");
    [[RTSPPreferencesController sharedController] showWindow:nil];
}

- (void)handleTestGoogleHomeCameras:(NSNotification *)notification {
    NSLog(@"[Menu] Test Google Home cameras");
    NSAlert *alert = [[NSAlert alloc] init];
    alert.messageText = @"Testing Cameras";
    alert.informativeText = @"Testing connectivity to all Google Home cameras...";
    alert.alertStyle = NSAlertStyleInformational;
    [alert addButtonWithTitle:@"OK"];
    [alert runModal];
}
...
- (void)showGoogleHomeCredentialsDialog {
    // Migrate existing secrets from NSUserDefaults to Keychain (if needed)
    ...
    NSAlert *alert = [[NSAlert alloc] init];
    alert.messageText = @"Google Home OAuth Configuration";
    alert.informativeText = @"Enter your Google OAuth credentials from Google Cloud Console:\n(Required for Google Home camera integration)";
    alert.alertStyle = NSAlertStyleInformational;

    // Create input fields
    NSTextField *clientIDLabel = [[NSTextField alloc] initWithFrame:NSMakeRect(0, 76, 400, 20)];
    clientIDLabel.stringValue = @"Client ID:";
    clientIDLabel.editable = NO;
    clientIDLabel.selectable = NO;
    clientIDLabel.bordered = NO;
    clientIDLabel.backgroundColor = [NSColor clearColor];

    NSTextField *clientIDField = [[NSTextField alloc] initWithFrame:NSMakeRect(0, 52, 400, 24)];
    clientIDField.placeholderString = @"123456789.apps.googleusercontent.com";
    clientIDField.stringValue = [defaults stringForKey:@"GoogleHome_ClientID"] ?: @"";

    NSTextField *clientSecretLabel = [[NSTextField alloc] initWithFrame:NSMakeRect(0, 28, 400, 20)];
    clientSecretLabel.stringValue = @"Client Secret:";
    clientSecretLabel.editable = NO;
    clientSecretLabel.selectable = NO;
    clientSecretLabel.bordered = NO;
    clientSecretLabel.backgroundColor = [NSColor clearColor];

    NSSecureTextField *clientSecretField = [[NSSecureTextField alloc] initWithFrame:NSMakeRect(0, 4, 400, 24)];
    ...
    NSString *savedSecret = [RTSPKeychainManager passwordForAccount:@"GoogleHome_ClientSecret"
                                                            service:RTSPKeychainServiceGoogleHome];
    clientSecretField.stringValue = savedSecret ?: @"";

    NSView *accessoryView = [[NSView alloc] initWithFrame:NSMakeRect(0, 0, 400, 100)];
    [accessoryView addSubview:clientIDLabel];
    [accessoryView addSubview:clientIDField];
    [accessoryView addSubview:clientSecretLabel];
    [accessoryView addSubview:clientSecretField];
    ...
    if ([alert runModal] == NSAlertFirstButtonReturn) {
        NSString *clientID = clientIDField.stringValue;
        NSString *clientSecret = clientSecretField.stringValue;
        ...
        if (clientID.length > 0 && clientSecret.length > 0) {
            ...
            googleAdapter.authentication.clientID = clientID;
            googleAdapter.authentication.clientSecret = clientSecret;
            [googleAdapter saveAuthentication];

            NSLog(@"[Menu] Google Home OAuth credentials saved");

            NSAlert *successAlert = [[NSAlert alloc] init];
            successAlert.messageText = @"Credentials Saved";
            successAlert.informativeText = @"Google OAuth credentials have been saved. You can now authenticate with Google Home.";
            successAlert.alertStyle = NSAlertStyleInformational;
            [successAlert addButtonWithTitle:@"OK"];
            [successAlert runModal];
        } else {
            ...
            errorAlert.informativeText = @"Please enter both Client ID and Client Secret.";
